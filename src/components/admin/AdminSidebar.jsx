...
import {
  ...
  PlayCircle, Headphones, ArrowLeft,
} from 'lucide-react';
...
const AdminSidebar = () => {
  const menuItems = [
    ...
    { to: '/admin/classes', label: 'ক্লাসসমূহ', icon: <PlayCircle size={20} /> },
  ...
  return (
    <aside style={{
      ...
      padding: '20px', boxShadow: '0 10px 40px rgba(108, 99, 255, 0.10)',
        ...
        {menuItems.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.end}
            style={({ isActive }) => ({
              ...
              background: isActive
                ? 'linear-gradient(135deg, #6C63FF, #5A52D5)' : 'transparent',
              textDecoration: 'none', transition: 'all 0.3s',
              boxShadow: isActive ? '0 8px 20px rgba(108, 99, 255, 0.25)' : 'none',
            })}
          >
            {item.icon}
            {item.label}
